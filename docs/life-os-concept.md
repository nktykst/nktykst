...
9. **RuView を置く部屋と WiFi** — 最初に監視したい部屋 (デスクのある部屋 / 寝室 / 両方)。ルーターは 2.4GHz を出しているか。ESP32-S3 を 2 台買って良いか。
10. **Claude Code をこの PC で使う頻度** — 開発と運用が同じサブスク枠を使うので、運用ジョブをどの時間帯に寄せるか。
